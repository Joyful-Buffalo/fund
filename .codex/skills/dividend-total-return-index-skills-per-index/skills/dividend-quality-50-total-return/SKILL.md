---
name: dividend-quality-50-total-return
description: 抓取 红利质量指数（50）全收益 的历史日线、日收益、累计收益、年化收益和最大回撤。只使用全收益/收益型代码，不使用价格指数、快照或拟合曲线兜底。
---
...
当用户要求抓取、更新、核验或导出 **红利质量指数（50）** 的全收益历史数据时，使用本 Skill。
...
```text
skills/dividend-quality-50-total-return/scripts/fetch_dividend_quality_50_total_return.py
```
...
```text
result/dividend_total_return_index_skills/dividend-quality-50-total-return/
├── dividend-quality-50-total-return.xlsx
├── csv/
│   ├── dividend-quality-50-total-return_daily.csv
│   ├── dividend-quality-50-total-return_summary.csv
│   └── errors.csv
└── png/
```
