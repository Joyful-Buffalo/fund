---
name: consumer-dividend-50-total-return
description: 抓取 消费红利指数（50）全收益 的历史日线、日收益、累计收益、年化收益和最大回撤。只使用全收益/收益型代码，不使用价格指数、快照或拟合曲线兜底。
---
...
当用户要求抓取、更新、核验或导出 **消费红利指数（50）** 的全收益历史数据时，使用本 Skill。
...
```text
skills/consumer-dividend-50-total-return/scripts/fetch_consumer_dividend_50_total_return.py
```
...
```text
result/dividend_total_return_index_skills/consumer-dividend-50-total-return/
├── consumer-dividend-50-total-return.xlsx
├── csv/
│   ├── consumer-dividend-50-total-return_daily.csv
│   ├── consumer-dividend-50-total-return_summary.csv
│   └── errors.csv
└── png/
```
